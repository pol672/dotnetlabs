...
namespace ClientSideApp
{
    ...
    public class DemoHttpClient : IDemoHttpClient
    {
        ...
        public async Task<string> UploadFile(string filePath)
        {
            ...
            var response = await _httpClient.PostAsync($"{_appUrl}/api/files", form);
        ...
        public async Task<string> DownloadFile(string guid)
        {
            ...
            var response = await _httpClient.GetAsync($"{_appUrl}/api/files?guid={guid}");
            response.EnsureSuccessStatusCode();
            ...
            _logger.LogInformation($"File saved as [{fileInfo.FullName}].");
